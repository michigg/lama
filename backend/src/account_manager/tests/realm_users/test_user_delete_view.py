...
class RealmUserDeleteViewTest(TestCase):
    ...
    def test_without_login(self):
        response = self.client.get(reverse('realm-user-delete', args=[self.realm_1.id, self.ldap_user_1.dn, ]))
    ...
    def test_with_admin_login(self):
        ...
        response = self.client.get(reverse('realm-user-delete', args=[self.realm_1.id, self.ldap_user_1.dn, ]))
        self.assertContains(response, f'<h3 class="text-danger">{self.ldap_user_1.username}', status_code=200)
        self.assertContains(response, f'Nutzer wurde als gelöscht markiert.', status_code=200)
        self.assertTrue(DeletedUser.objects.filter(ldap_dn=self.ldap_user_1.dn).exists())
    ...
    def test_with_admin_login_already_deleted_user(self):
        ...
        self.client.login(username=self.ldap_user_realm_1_admin.username, password=get_password())
        response = self.client.get(reverse('realm-user-delete', args=[self.realm_1.id, self.ldap_user_1.dn, ]))
        self.assertContains(response, f'<h3 class="text-danger">{self.ldap_user_1.username}', status_code=409)
        self.assertContains(response, f'Nutzer ist bereits als gelöscht markiert.', status_code=409)
        self.assertTrue(DeletedUser.objects.filter(ldap_dn=self.ldap_user_1.dn).exists())
