...
class RealmUserUpdateViewTest(TestCase):
    ...
    def test_without_login(self):
        response = self.client.get(reverse('realm-user-update', args=[self.realm_1.id, self.ldap_user_1.dn, ]))
    ...
    def test_with_admin_login(self):
        ...
        response = self.client.get(reverse('realm-user-update', args=[self.realm_1.id, self.ldap_user_1.dn, ]))
        self.assertContains(response, f'<h3>{self.ldap_user_1.username}</h3>', status_code=200)

    def test_with_admin_login_post_new_user_data_only_username_email(self):
        self.client.login(username=self.ldap_user_realm_1_admin.username, password=get_password())
        new_username = "new_test_user"
        new_email = "test@test.de"
        response = self.client.post(reverse('realm-user-update', args=[self.realm_1.id, self.ldap_user_1.dn, ]),
                                    data={'username': new_username, "email": new_email})

        self.assertContains(response, f'<h3>{self.ldap_user_1.username}</h3>', status_code=200)
        self.assertContains(response, 'field is required')
        self.assertFalse(LdapUser.objects.filter(username=new_username, email=new_email).exists())

    def test_with_admin_login_post_new_user_data(self):
        self.client.login(username=self.ldap_user_realm_1_admin.username, password=get_password())
        new_username = "new_test_user"
        new_email = "test@test.de"
        new_first_name = "Test"
        new_last_name = "Test"
        response = self.client.post(reverse('realm-user-update',
                                            args=[self.realm_1.id, self.ldap_user_1.dn, ]),
                                    data={
        ...
        self.assertContains(response, f'<h3>{new_username}</h3>', status_code=200)
        self.assertTrue(LdapUser.objects.filter(username=new_username,
                                                email=new_email,
                                                first_name=new_first_name,
                                                last_name=new_last_name).exists())
    ...
    def test_with_super_user_login_post_user_data_only_username_email(self):
        self.client.login(username=self.django_superuser.username, password=get_password())
        new_username = "new_test_user"
        new_email = "test@test.de"
        response = self.client.post(reverse('realm-user-update', args=[self.realm_1.id, self.ldap_user_1.dn, ]),
    ...
    def test_with_super_user_login_post_new_user_data(self):
        self.client.login(username=self.django_superuser.username, password=get_password())
        new_username = "new_test_user"
        new_email = "test@test.de"
        new_first_name = "Test"
