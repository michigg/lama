...
class RealmUserDeleteConfirmViewTest(TestCase):
    ...
    def test_without_login(self):
        response = self.client.get(reverse('realm-user-delete-confirm', args=[self.realm_1.id, self.ldap_user_1.dn, ]))
    ...
    def test_with_admin_login(self):
        ...
        self.assertContains(response,
                            f'<strong>Achtung!</strong> Sie sind gerade dabei den Account von <strong>{self.ldap_user_1.username}</strong> zu schließen.',
                            status_code=200)
        self.assertContains(response,
                            f'Der Account bleibt noch {settings.DELETION_WAIT_DAYS} Tage (vom heutigen Tag aus)',
                            status_code=200)
