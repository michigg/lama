...
class RealmUserDeleteViewTest(TestCase):
    ...
    def test_without_login(self):
        response = self.client.get(reverse('realm-user-direct-delete', args=[self.realm_1.id, self.ldap_user_1.dn, ]))
    ...
    def test_with_admin_login_with_missing_deletion_marker(self):
        self.client.login(username=self.ldap_user_realm_1_admin.username, password=get_password())
        response = self.client.get(reverse('realm-user-direct-delete', args=[self.realm_1.id, self.ldap_user_1.dn, ]))
        self.assertContains(response, f'Löschen erfolgreich', status_code=200)
        self.assertContains(response, f'Nutzer {self.ldap_user_1.username} wurde erfolgreich gelöscht.',
                            status_code=200)
        self.assertFalse(User.objects.filter(username=self.ldap_user_1.username).exists())
        self.assertFalse(LdapUser.objects.filter(username=self.ldap_user_1.username).exists())
        self.assertFalse(DeletedUser.objects.filter(ldap_dn=self.ldap_user_1.dn).exists())

    def test_with_admin_login_already_deleted_user(self):
    ...
    def test_with_super_user_login_with_missing_deletion_marker(self):
    ...
    def test_with_super_user_login_already_deleted_user(self):
        user = User.objects.get(username=self.ldap_user_1.username)
        DeletedUser.objects.create(user=user, ldap_dn=self.ldap_user_1.dn)
        self.client.login(username=self.django_superuser.username, password=get_password())
        response = self.client.get(reverse('realm-user-direct-delete', args=[self.realm_1.id, self.ldap_user_1.dn, ]))
